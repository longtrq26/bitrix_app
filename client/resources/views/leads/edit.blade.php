...
@section('content')
    <div class="space-y-6">
        ...
        <h1 class="text-2xl font-bold">Chỉnh sửa Lead #{{ e($id) }}</h1>

        <!-- Form cập nhật lead -->
        <form method="POST" action="{{ route('leads.update', $id) }}"
            class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow" id="update-lead-form">
            @csrf
            @method('PATCH')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium dark:text-gray-300">Tiêu đề *</label>
                    <input type="text" name="TITLE" value="{{ e(old('TITLE', $lead['TITLE'] ?? '')) }}" required
                        maxlength="100"
                ...
                <div>
                    ...
                    <input type="email" name="EMAIL"
                        value="{{ e(old('EMAIL', isset($lead['EMAIL'][0]['VALUE']) ? $lead['EMAIL'][0]['VALUE'] : '')) }}"
                        class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500">
                    @error('EMAIL')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="PHONE"
                        value="{{ e(old('PHONE', isset($lead['PHONE'][0]['VALUE']) ? $lead['PHONE'][0]['VALUE'] : '')) }}"
                        class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    ...
                    <select name="STATUS_ID"
                        ...
                        @foreach ($statuses ?? [] as $status)
                            <option value="{{ e($status['STATUS_ID']) }}" {{ old('STATUS_ID', $lead['STATUS_ID'] ?? '') == $status['STATUS_ID'] ? 'selected' : '' }}>
                                {{ e($status['NAME']) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="SOURCE_ID"
                        ...
                        @foreach ($sources ?? [] as $source)
                            <option value="{{ e($source['STATUS_ID']) }}" {{ old('SOURCE_ID', $lead['SOURCE_ID'] ?? '') == $source['STATUS_ID'] ? 'selected' : '' }}>
                                {{ e($source['NAME']) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="COMMENTS" rows="3" maxlength="1000"
                        class="w-full border p-2 rounded dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500">{{ e(old('COMMENTS', $lead['COMMENTS'] ?? '')) }}</textarea>
                    @error('COMMENTS')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    @push('scripts')
        <script>
            document.getElementById('update-lead-form').addEventListener('submit', function () {
